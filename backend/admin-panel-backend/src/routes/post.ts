...
class StudentController implements IController {
    ...
    createPost = async (req: ExtendedRequest, res: Response) => {
        try {
            ...
            if (delivery_at) {
                if (!/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(delivery_at)) {
                    throw {
                        status: 400,
                        message: 'Invalid delivery_at format'
                    };
                }
                deliveryAtValue = delivery_at; // Use directly, assuming UTC from client
            }
    ...
    postList = async (req: ExtendedRequest, res: Response) => {
        try {
            ...
            filters.push('(po.delivery_at IS NULL OR po.delivery_at <= UTC_TIMESTAMP())');
    ...
    scheduledPostList = async (req: ExtendedRequest, res: Response) => {
        try {
            ...
            const filters = ['po.delivery_at IS NOT NULL AND po.delivery_at > UTC_TIMESTAMP()'];
            const params: any = {
                school_id: req.user.school_id,
                limit: limit,
                offset: offset
    ...
    postUpdate = async (req: ExtendedRequest, res: Response) => {
        try {
            ...
            await DB.execute(`UPDATE Post SET
                ...
                priority = :priority, edited_at = UTC_TIMESTAMP()
            WHERE id = :id AND school_id = :school_id`, {
                id: post.id,
                school_id: req.user.school_id,
                title: title,
